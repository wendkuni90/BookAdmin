...
<?php require "scripts/borrows_scripts/total_Borrows.php" ?>
						...
						<?php if(!empty($borrows)): ?>
							<?php foreach ($borrows as $borrow): ?>
								<tr>
                                    ...
                                    <td>
                                        <?= htmlspecialchars($borrow['borrow_return']); ?>
                                    </td>
                                    <td>
                                        <?= htmlspecialchars($borrow['borrow_status']); ?>
                                    </td>
									<td style="display:flex;">
                                        <a href="scripts/edit_borrow.php?id=<?=$borrow['borrow_id'];?>" style="width:32px; height:32px; margin-right:5px">
                                                <img src="../assets/img/edit.svg" alt="">
                                        </a> 
										<div style="width:1px;height:22px;background-color:black;"></div>
										<a href="scripts/delete_borrow.php?id=<?=$borrow['borrow_id'];?>" onclick="return confirm('Supprimer emprunt ?');" style="width:32px; height:32px;margin-left:5px">
